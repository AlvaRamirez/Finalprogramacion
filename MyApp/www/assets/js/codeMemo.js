...
/*function actualizarPuntajes(){
    var puntaje1= parseInt(localStorage.getItem("puntajeGlobal1"));
    var puntaje2= parseInt(localStorage.getItem("puntajeGlobal2"));

    localStorage.setItem("puntajeGlobal1",puntaje1+scorememo[0]);
    localStorage.setItem("puntajeGlobal2",puntaje2+scorememo[1]);
...
function drawBoard(selectedSize) {
    ...
    for (let i = 0; i < boardSize; i++) {
        for (let j = 0; j < boardSize; j++) {
            ...
            container.append("<div id='" + tile + "' class='flip-box' onmouseover='hover(" + JSON.stringify(tile) + ")' onclick='clickTile(" + JSON.stringify(tile) + ")'><div class='flip-box-inner'><div class='flip-box-front'><imgMemo src='' alt=''/></div><div class='flip-box-back'><img id='img" + board[i][j].ficha + "' src='" + board[i][j].imagen + "'></div></div></div>");
        }
    }
           ...
           // playerTurn.points += turnsInARow * 100;
            ...
            if (pairsFound === totalPairs) {
                ...
                if (player1.pairs > player2.pairs) {
                    winDiv.innerHTML = '<h1>¡Ganó ' + jugador1 +'!</h1><span onclick="restart()">Empezar nueva partida</span>';
                    winDiv.className = "appear";
                    scorememo[0]=scorememo[0]+150;
                    console.log("jugador 1= " + scorememo[0]);
                } else {
                    if (player1.pairs === player2.pairs) {
                        winDiv.innerHTML = '<h1>¡Empate!</h1><span onclick="restart()">Empezar nueva partida</span>';
                        winDiv.className = "appear";
                    } else {
                        winDiv.innerHTML = '<h1>¡Ganó ' + jugador2 +'!</h1><span onclick="restart()">Empezar nueva partida</span>';
                        winDiv.className = "appear";
                        scorememo[1]=scorememo[1]+150;
                        console.log("jugador 2= " + scorememo[1]);
                    }
                }
            }
        } else {
...
function setTilesLogicalBoard(boardSize) {
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        board[positions[i].row][positions[i].col] = {
            ...
            imagen: "./assets/img/imgMemo/" + ficha + ".jpg", //SRC DE LA IMG
        };
...
function areYouSure(){
    $("#msgBox").removeClass("none");
     $("#msgBox").append('<div><h3>¿Seguro que quieren reiniciar?</h3><button onclick="msgBoxDone1(1)">Sí, deseamos reiniciar</button><button onclick="msgBoxDone2()">No, queremos seguir jugando</button></div>');
...
function restart() {
    ...
    $("#turn").text("Selecciona un tablero");
    disableAll = false;
    ...
    document.getElementById("selectSize").value="";
